...
1. Download the data102 `data102_environment.yml`] from the course repository 
   [here](https://raw.githubusercontent.com/DS-102/TODO) or:

        # download via curl
        curl https://raw.githubusercontent.com/DS-102/TODO > data102_environment.yml

        # OR download via wget
        wget -O data102_environment.yml https://raw.githubusercontent.com/DS-102/TODO
...
1. Using the **Terminal**, navigate to the directory where you downloaded `data102_environment.yml`. Run these commands to create a new conda environment. Each conda environment maintains its own package versions, allowing us to switch between package versions easily. For example, this class uses Python 3, but you might have another that uses Python 2. With a conda environment, you can switch between those at will.
        ...
        conda env create -f data102_environment.yml

        # Switch to the data102 environment
        conda activate data102
		
		# Check if packages are in the environment
		# This should not be empty!
		conda list

From now on, you can switch to the `data102` env with `conda activate data102`, and switch back to the default env with `conda deactivate`. 
...
Make sure to **always** work in the `data102` conda environment when you are using jupyter notebooks for this class. This ensures you have all the necessary packages required for the notebook to run.

## Verifying Your Environment

You can tell if you are correct environment if your terminal looks something like:

![conda-env](assets/local_setup/data102_environment.PNG)

Additionally,
    
    conda env list
    
outputs a list of all your conda environments, and `data102` should appear with a `*` next to it (the active one).
...
If you feel as if you've messed up and need to start over, you can remove the environment with
	...
	conda remove --name data102 --all
	
To verify that the environment was removed, in your Terminal window or an Anaconda Prompt, run:

	conda info --envs

Which should then no longer display the `data102` environment.
